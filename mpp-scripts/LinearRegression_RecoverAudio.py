...
# Provided_Portion parameter to experiment with how much of the audio to chop off
# and will be generated via linear extrapolation.
Provided_Portion = 0.25

#Load up all 50 of the 0_jackson*.wav
zero = []
path = "C:/Users/Atiqa.Zafar/Downloads/free-spoken-digit-dataset-master/free-spoken-digit-dataset-master/recordings/"
for file in os.listdir(path):
    if fnmatch.fnmatch(file, '0_jackson*.wav'):
        sample_rate, audio_data = wavfile.read(path+file)
        zero.append(audio_data)
        
        
# convert zero into a DataFrame. When you do so, set the dtype to np.int16, 
#since the input audio files are 16 bits per sample. 
...
# There are 50 takes of each clip. You want to pull out just one
...
# INFO: train will be shaped: [n_samples, n_audio_samples], where
...
# you've generated it. You should have got the sample_rate
...
#Use your model to predict the 'label' of X_test. 
y_test_prediction = model.predict(X_test)
print(y_test_prediction.dtype)
...
# Unfortunately, even the linear regression algorithm knew how poorly it did. 
#It's goal was to extrapolate more than three times further than the range of the training data. 
#Linear regression really isn't the best algorithm to use for this type of work, but even so, 
#the audio file produced is clearly distinguishable, although it sounds a little compressed and 
#like a few people are talking simultaneously.
